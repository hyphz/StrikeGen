...
buddyPassiveChoices m = mayList (getResponse m "buddy-p1") ++
                        atLevelList m 5 (mayList (getResponse m "buddy-p2")) ++
                        atLevelList m 9 (mayList (getResponse m "buddy-p3"))
...
buddyText m = buddyHitPoints m ++ String.concat (List.map (buddyPassiveDescriptor m) (Dict.keys buddyPassives)) ++
...
basePowers m = [quickPower "Team Attack" Attack AtWill -10 0 (atWillDamage m) Green m,
                ...
                quickPower "Boosted Buddy" Attack Encounter 0 0 3 Green m]



powers m = basePowers m ++ [buddySpecial m]
