...
commandUndead m =
    ...
    , text =
        (if (getLevel m) < 9 then
            (overtext m "CommandUndead")
         else
            (overtext m "CommandUndead9+")
        )
    , slot = Attack
    , freq = Encounter
    , range = 10
...
lifeDrain =
    quickPower "Life Drain" Attack Encounter 0 0 2 Purple


corpseExplosion =
    quickPower "Corpse Explosion" Attack Encounter 0 0 0 Purple


raiseAlly =
    quickPower "Raise Ally" Misc Encounter 0 0 0 Purple


seedOfFear : Model -> Power
seedOfFear =
    quickPower "Seed Of Fear" Attack Encounter 5 0 3 Purple


greaterMarkOfDeath : Model -> Power
greaterMarkOfDeath m =
    { name = "Greater Mark Of Death"
    , text =
        case (getResponse m "necro-gift") of
            Just "Undeath" ->
                overtext m "GreaterMarkOfDeathWithUndeath"

            _ ->
                overtext m "GreaterMarkOfDeath"
    , slot = Attack
    , freq = Encounter
    , range = -5
    , area = 0
...
speakWithDead m =
    quickPower "Speak With Dead" Attack AtWill -5 0 (atwilldamage m) Green m
    ...
    case (getResponse m "necro-gift") of
        ...
        Just gift ->
            let
                ...
                giftTextKey =
                    giftLevelDesc ++ "GiftOf" ++ gift

                giftName =
                    giftLevelDesc ++ " Gift Of " ++ gift
...
l1atwills m =
    powerDict m [ deadlyPoison, phantasms, terrifyingVisage ]


l1atwillpower1 m =
    powerlookup m "necro-aw1" l1atwills


l1atwillpower2 m =
    powerlookup m "necro-aw2" l1atwills


l1encoptions m =
    powerDict m [ lifeDrain, corpseExplosion, raiseAlly, seedOfFear ]


l3encoptions m =
    powerDict m [ greaterMarkOfDeath, lichPact, healthSwap ]


l7encoptions m =
    powerDict m [ crudeDomination, armyOfSpecters, playDiceWithDeath, terror ]


l1encpower m =
    powerlookup m "necro-enc" l1encoptions


l3encpower m =
    powerlookup m "necro-enc3" l3encoptions


l7encpower m =
    powerlookup m "necro-enc7" l7encoptions


necroPowers m =
    [ commandUndead m, markOfDeath m, speakWithDead m ]
        ++ (giftPower m)
        ++ (l1atwillpower1 m)
        ++ (l1atwillpower2 m)
        ++ (l1encpower m)
