...
summonChoiceField m typelookup index =
    case (getResponse m typelookup) of
        ...
        Just typeName ->
            case (Dict.get typeName summonTypeNames) of
                Nothing ->
                    []
...
summonPowerBlockDirect m summonType index =
    case (getResponse m (summonFieldName summonType index)) of
        ...
        Just summonName ->
            case (Dict.get summonName allSummonsDict) of
                Nothing ->
                    []

                Just summon ->
                    [ PowerBlock summonName [ summon.summon m, summon.atwill m, summon.creature m ] ]
...
eggpower m =
    case (getResponse m "basics-name") of
        ...
        Just x ->
            if ((String.toLower x) == "bmx bandit!") then
...
powers m =
    [ levelTextSpecial "Summoning" [ 1, 5 ] m ]
        ...
        ++ if (hasFeat m "Reliable Summoner") then
            [ quickSpecial "Reliable Summoner" m ]
           else
            []
