...
startStance =
    quickSpecial "Stances"


changeStance =
    quickPower "Change Stance" Special AtWill 0 0 0 Yellow


focusedAttack =
    quickPower "Focused Attack" Special Encounter 0 0 0 Purple


quickStance m n =
    quickPower n Special None 0 0 0 Yellow m
...
decapitation =
    quickPower "Decapitation" Attack Encounter 0 0 3 Purple


theFalseDeath =
    quickPower "The False Death" Special Encounter 0 0 0 Purple


martialFrenzy =
    quickPower "Martial Frenzy" Special Encounter 0 0 0 Purple


extraFocusedAttack =
    quickPower "Extra Focused Attack" Special Encounter 0 0 0 Purple


genStances m =
    let
        stanceChoices =
            mayList (getResponse m "ma-s1")
                ++ mayList (getResponse m "ma-s2")
                ++ mayList (getResponse m "ma-s3")

        allStances x =
            [ quickStance m (x ++ " Style")
            , quickStance m ("Greater " ++ x ++ " Style")
            , quickStance m ("Supreme " ++ x ++ " Style")
            ]
    in
        (List.concatMap (allStances) stanceChoices)


l3encounters m =
    powerDict m [ extraFocusedAttack, decapitation, theFalseDeath ]


l3encpower m =
    powerlookup m "ma-enc3" l3encounters


l7encounters m =
    powerDict m [ extraFocusedAttack, decapitation, theFalseDeath, martialFrenzy ]


l7encpower m =
    powerlookup m "ma-enc7" l7encounters


mamaster m =
    quickSpecial "Master Martial Artist" m
...
eggpower m =
    case (getResponse m "basics-name") of
        ...
        Just x ->
            if ((String.toLower x) == "daniel-san!") then
...
oppPower m =
    let
        ...
        oppText =
            "Apply the effects of your Style when you deal Opportunity damage."
                ++ (if (List.member "Python" stanceChoices) then
                        " If you are in Python style, the enemy gets an immediate escape attempt for free to avoid your Grab."
                    else
                        ""
                   )
    in
...
powers m =
    genStances m
        ++ [ startStance m, changeStance m, focusedAttack m ]
        ++ (atLevelList m 3 (l3encpower m))
        ++ (atLevelList m 7 (l7encpower m))
        ++ (atLevel m 9 (mamaster m))
        ++ (if (hasFeat m "Opportunistic Martial Artist") then
                [ oppPower m ]
            else
                []
           )
