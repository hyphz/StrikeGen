...
shapeBbpower m k shape =
    if (lacksFeat m "Shapechanger Bread and Butter") then
        []
    else
        case (getResponse m (k ++ "-bb")) of
            ...
            Just atWillname ->
                if (not (List.member atWillname (shapeAtwillNames m k))) then
                    []
                else
                    [ Power "Bread and Butter" ("When any power or ability allows you to make a Basic attack while you are in " ++ shape.name ++ " form, you may use " ++ atWillname ++ " instead.") Misc None 0 0 0 White ]
...
shapeRolePowers m k =
    if (lacksFeat m "Multi-Role Shapechanger") then
        []
    else
        case (getResponse m (k ++ "-role")) of
            ...
            Just roleName ->
                case (Dict.get roleName roles) of
                    Nothing ->
                        []

                    Just role ->
                        role.rolePowerListPrefix m (k ++ "-") ++ [ quickSpecial "Multi-Role Shapechanger" m ]
...
shapePowerBlock m k =
    case (getResponse m k) of
        ...
        Just shapeName ->
            case (Dict.get shapeName shapes) of
                Nothing ->
                    []
                ...
                Just shape ->
                    [ { name = shapeName
                      , powers =
                            [ shape.transform m, shape.special m, shape.atwill1 m, shape.atwill2 m ]
    ...
    case (getResponse m k) of
        ...
        Just shapeName ->
            case (Dict.get shapeName shapes) of
                ...
                Just shape ->
                    [ .name (shape.atwill1 m), .name (shape.atwill2 m) ]
...
shapeChoiceField m name key list =
    [ DropdownField { name = name, del = False, key = key, choices = [ "" ] ++ (Dict.keys (list)) } ]
        ++ (if (hasFeat m "Multi-Role Shapechanger") then
                [ DropdownField { name = "Role:", del = False, key = (key ++ "-role"), choices = [ "" ] ++ (Dict.keys (roles)) } ]
            else
                []
           )
        ++ (if (hasFeat m "Shapechanger Bread and Butter") then
                [ DropdownField { name = "Bread and Butter:", del = False, key = (key ++ "-bb"), choices = [ "" ] ++ (shapeAtwillNames m key) } ]
            else
                []
           )
...
modifyRally m p =
    case (getResponse m "shaper-type") of
        Just "One-Form" ->
            { p | text = overtext m "OneFormShaperRally" }

        Just "Multi-Form" ->
            { p | text = overtext m "MultiFormShaperRally" }
...
powers m =
    [ quickPower "Blurred Form" Attack AtWill 0 0 2 Green m
    , quickPower "Primal Compulsion" Attack AtWill 10 0 2 Green m
    ]
        ++ case (getResponse m "shaper-type") of
            Just "One-Form" ->
                [ quickSpecial "One Form Shaper" m ]

            Just "Multi-Form" ->
                [ quickSpecial "Multi Form Shaper" m ]
...
powerBlocks m =
    case (getResponse m "shaper-type") of
        Just "One-Form" ->
            shapePowerBlock m "shaper-shape1"

        Just "Multi-Form" ->
            List.concatMap (\x -> shapePowerBlock m x) [ "shaper-shape1", "shaper-shape2", "shaper-shape3" ]
                    ...
                    case (getResponse m k) of
                        ...
                        Just shapeName ->
                            case (Dict.get shapeName shapes) of
                                ...
                                Just shape ->
                                    List.map (\x -> { x | name = x.name ++ " (" ++ shapeName ++ ")" }) (role.roleFormsPrefix m (k ++ "-"))
...
forms m =
    [ Form False
        ...
        ([ DropdownField { name = "Type:", key = "shaper-type", del = False, choices = [ "", "Multi-Form", "One-Form" ] }
            ...
            ++ case (getResponse m "shaper-type") of
                Just "Multi-Form" ->
                    shapeChoiceField m "Form:" "shaper-shape1" shapes
