...
simpleDamageEffect m =
  if (getLevel m) < 3 then "**Effect**: 1 damage." else
  if (getLevel m) < 7 then "**Effect**: 2 damage." else
     "**Effect**: 3 damage."

simpleDamageValue m =
  if (getLevel m) < 5 then 2 else
  if (getLevel m) < 9 then 3 else 4
...
forms m = [Form False "Simplified Class" ([
