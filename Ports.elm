...
-- OUTGOING
port download : (String, String) -> Cmd x -- Send the string as a file with download.js .
port saveURL : String -> Cmd x -- Pack the string as a parameter and reload the page.
port dbLoaded : Int -> Cmd x -- Junk parameter. Called when loading is done to tell the JS that
