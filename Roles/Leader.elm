...
actionTrigger m =
    if (getLevel m) < 6 then
        [ quickPower "Try again!" Reaction Encounter 0 0 0 Yellow m ]
    else
        [ quickPower "Try harder!" Reaction Encounter 0 0 0 Yellow m ]


boosts m =
    case (hasFeat m "Limber Leader") of
        False ->
            [ levelTextPower "Tactics" RoleSlot Encounter 0 0 0 Red [ 1, 4, 8 ] m
            , levelTextPower "Heal" RoleSlot Encounter 5 0 0 Red [ 1, 4, 8 ] m
            ]

        True ->
            [ levelTextPower "Limber Tactics" RoleSlot Encounter 0 0 0 Red [ 1, 4, 8 ] m
            , levelTextPower "Heal" RoleSlot Encounter 5 0 0 Red [ 1, 4, 8 ] m
            ]
...
upgraded x m =
    case x of
        "Hit 'Em!" ->
            [ quickPower "Hit 'Em Harder!" RoleSlot Encounter 0 0 0 Red m ]

        "Walk it off!" ->
            [ quickPower "Walk it off harder!" RoleSlot Encounter 0 0 0 Red m ]

        "Mass Heal" ->
            [ quickPower "Fresh Start" RoleSlot Encounter 0 0 0 Red m ]

        "Keep Walking!" ->
            [ quickPower "You're Fine!" Reaction Encounter 0 0 0 Red m ]

        "Reveal Weakness" ->
            [ quickPower "Reveal Weak Point" Reaction Encounter 0 0 0 Red m ]
...
checkUpgrade m pr p =
    if ((getLevel m) < 10) then
        ...
        case (List.head p) of
            ...
            Just rp ->
                case (getResponse m (pr ++ "leader-upgrade")) of
                    Nothing ->
                        p
...
l2encchosen m p =
    checkUpgrade m p (prefixpowerlookup m p "leader-enc1" encounters)


l6encchosen m p =
    checkUpgrade m p (prefixpowerlookup m p "leader-enc2" encounters)


upgradable m p =
    [ "" ]
        ++ (List.map .name
                (prefixpowerlookup m p "leader-enc1" encounters
...
eggpower m =
    case (getResponse m "basics-name") of
        ...
        Just x ->
            if ((String.toLower x) == "call me dave!") then
